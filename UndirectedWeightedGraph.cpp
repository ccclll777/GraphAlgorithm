...
bool UndirectedWeightedGraph::addEdge() {
...
bool UndirectedWeightedGraph::deleteEdge() {
    cout << "请输入要删除的边" << endl;
    int from, to;
    cin >> from;
    cin >> to;
    if ((from < 0 || from > vertex) && (to < 0 || to > vertex)) {
        ...
        m_pWeightMatrix[to][from] = NoEdge;
        return true;
    }
}

void UndirectedWeightedGraph::printWeightMatrix() {
    cout << "此图的权重矩阵为：" << endl;
...
void UndirectedWeightedGraph::Prim() {
    cout<<endl;
...
void UndirectedWeightedGraph::getWeight() {
    cout<<endl;
